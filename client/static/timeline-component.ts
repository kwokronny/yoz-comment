...
export class YozTimeLineComponent {
  public element: HTMLDivElement;
  private moreDOM: HTMLDivElement;
  private page: number = 1;
  private loading: boolean = false;
  private commentComp: YozCommentComponent;
  public constructor() {
    this.element = document.createElement("div");
    this.element.className = "yoz-comment-list";
    this.commentComp = new YozCommentComponent();
  ...
  public getList(page?: number) {
    ...
    let params = {
      token: window.YozCommentConfig.token,
      page,
    };
    axios
      .get("api/page", {
  ...
  private renderCommentItem(list: YozCommentItem[], first: number = 0): string {
    return list.reduce((html: string, item: YozCommentItem) => {
